...
export default function TargetAssignedDistributionContainer() {
    ...
    const [assignedDistribution, setAssignedDistribution] = useState<Distribution | undefined>();

    useEffect(() => {
        if (!selectedTarget) return;

        const fetchAssignedDistribution = async () => {
            const attributes = await TargetsService.getAssignedDistribution(selectedTarget.controllerId);
    ...
    return <TargetAssignedDistribution type={assignedDistribution?.typeName} version={assignedDistribution?.version} name={assignedDistribution?.name} />;
}
