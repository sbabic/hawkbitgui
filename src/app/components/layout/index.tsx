        ...
        <div className={styles.layout}>
            ...
            <aside className={`${styles.sidebar} ${sidebarOpen ? "" : styles.collapsed}`}>
                <div className={styles.sidebarHeader}>
                    <a className={styles.brand}><Image width={93} height={42} src={'/images/hawkbit-logo.svg'} alt={'Hawkbit logo'}/></a>
                </div>

                <nav className={styles.nav}>
                    {navItems.map((item, i) => <NavItem href={item.href} key={i} icon={item.icon} expanded={sidebarOpen} label={item.label} isActive={item.href === pathname} />)}
                </nav>

                <div className={styles.logout}>
                    <NavItem href="/logout" label="Logout" icon={LogoutIcon} expanded={sidebarOpen} />
                </div>
            </aside>
