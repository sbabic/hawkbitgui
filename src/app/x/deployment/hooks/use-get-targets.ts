...
const fetchTargets = async (): Promise<TargetsResponse> => {
    const response = await axiosInstance.get<TargetsResponse>('/targets');
    return response.data;
};

export const useGetTargets = () => {
    const { data, isLoading, error } = useQuery<TargetsResponse, ApiError>({
        queryKey: ['targets'],
